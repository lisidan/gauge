...
func TestHandleRerunFlagsWithVerbose(t *testing.T) {
	...
	cmd.Flags().Set("repeat", "true")
	cmd.Flags().Set(verboseName, "true")

	handleFlags(cmd)
	lastFlagSet := os.Args[len(os.Args)-1]
	expectedFlag := "--verbose=true"

	if !reflect.DeepEqual(lastFlagSet, expectedFlag) {
		t.Errorf("Expected %v  Got %v", expectedFlag, lastFlagSet)
	}
}
